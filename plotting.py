...
g.set_axis_labels('', '% of Participants', fontproperties=prop, fontsize=20)
g.set_titles(col_template="{col_name}", fontproperties=prop, size=25)
g.set(xlim=(0, 1))
g.set_xticklabels(fontproperties=prop)
g.set_yticklabels(fontproperties=prop)
g.fig.text(0.5, 0.05, 'Subjective Dirichlet Weight', ha='center', fontproperties=prop, fontsize=20)
...
sns.set_style('white')
sns.barplot(x='Condition', y='As', data=all_mean_CA, color=sns.color_palette('deep')[0], order=['LV', 'MV', 'HV'], errorbar='ci')
plt.ylabel('% Selecting A in CA Trials', fontproperties=prop, fontsize=20)
plt.xlabel('')
plt.gca().yaxis.set_major_formatter(ticker.PercentFormatter(xmax=1))
plt.xticks(fontproperties=prop, fontsize=15)
plt.yticks(fontproperties=prop, fontsize=15)
plt.axhline(y=(0.65/1.4), color='black', linestyle='-', linewidth=1, label='Reward Ratio')
